...
- **Non-goal:** auth of any kind — no password hashing, no sessions, no login endpoint. The service stays localhost-only for now ([ADR 0001](../decisions/0001-surfaces-and-stack.md)). **Still true as of 2026-08-29, and now for a scheduled reason:** [ADR 0007](../decisions/0007-real-auth-open-signup-hosted-later.md) made auth real and [backlog 0008](0008-api-auth-signup-login-sessions.md) owns it, adding `users.password_hash` and `sessions` in migration `0002`. **This ticket still does not build auth** — it builds the ownership model 0008 supplies a user to, and the `users` table keeps no password column. **This is now a narrower non-goal than it reads:** [ADR 0004](../decisions/0004-multi-user-tenant-scoped-from-day-one.md) made the product multi-user on 2026-08-25, so the *schema* is tenant-scoped from this ticket onward even though the *auth mechanics* are still out of scope. Build the ownership model; do not build the login.
...
2. **Phase 2 — `api`:** migration `0001` creating — **every user-data table carries `user_id`, not nullable, per [ADR 0004](../decisions/0004-multi-user-tenant-scoped-from-day-one.md):**
   ...
   - `accounts` — `id`, **`user_id`**, `name`, `type`, `opening_balance_minor` (`bigint`), `archived` (boolean, default false), `created_at`. **`archived` added 2026-08-29** — the client has shipped it since 0004 (see the section at the foot of this ticket), and an account with transactions can never be deleted without orphaning history, so archive is the only destructive edge that exists.
   ...
   - `transactions` — `id`, **`user_id`**, `account_id`, `category_id` (nullable), `amount_minor` (`bigint`, signed), `occurred_on` (`date`), `description`, `transfer_id` (nullable), `created_at`
   - **`transfer_id` is unblocked as of 2026-08-29** by [ADR 0006](../decisions/0006-transfers-linked-pair-invariant-in-db.md), which settled the shape this ticket was told to wait for. Transfers are a **linked pair** of rows sharing a `transfer_id`, and **the invariant is enforced by the database, not by callers**: a `DEFERRABLE INITIALLY DEFERRED` constraint that the two legs sum to zero, share `occurred_on` and `user_id`, sit on **different** accounts, and that a `transfer_id` has **exactly two** legs. Say in the outcome which constraint form you used, and **demonstrate a rejected bad pair** — a passing happy path proves nothing about a constraint.
...
- **`accounts.archived`** — boolean. **Folded into phase 2 on 2026-08-29.** An account with transactions can never be deleted without orphaning history, so archive is the only destructive edge the client offers, and it is reversible. The client chose `archived: boolean` over `archived_at` deliberately: the UI needs only *whether*, and a timestamp nothing renders is speculative modelling. Widening it later is cheap.
- **`transactions.transfer_id`** — ~~do not write this column until the transfer ADR exists~~. **Resolved 2026-08-29: [ADR 0006](../decisions/0006-transfers-linked-pair-invariant-in-db.md) accepted the linked pair and put the invariant in the database**, on the evidence 0004 produced. Folded into phase 2 above; this is no longer a blocker.
